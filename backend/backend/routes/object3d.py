...
def create_object3d(object3d: schemas.Object3DCreate, db: Session = Depends(get_db)):
    ...
    db_object3d = models.Object3D(name=object3d.name, uuid=uuid.uuid4(), floor_id = floor.uuid)
    db.add(db_object3d)
    db.commit()
...
def read_object3d(object3d_id: UUID, db: Session = Depends(get_db)):
    db_object3d = db.query(models.Object3D).filter(models.Object3D.uuid == object3d_id).first()
